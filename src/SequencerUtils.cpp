...
SequencerEditor::SequencerEditor(Sequencer *sequencer) : sequencer{sequencer}, currentSequence{0}, currentStep{0}, currentStepRow{0}, currentStepCol{0}, editMode{SequencerEditorMode::selectingSeqAndStep}, editSubMode{SequencerEditorSubMode::editCol1}, stepIncrement{0.5f}
{
}
...
void SequencerEditor::moveCursorRight()
{
  ...
  case SequencerEditorMode::editingStep:
  {
    ...
    currentStepCol ++;
    int maxCols = sequencer->getStepDataDirect(currentSequence, currentStep)->at(currentStepRow).size();
...
void SequencerEditor::moveCursorDown()
{
  ...
  {
    currentStepRow += 1;
    int rowsInStep = sequencer->getStepDataDirect(currentSequence, currentStep)->size();
...
void SequencerEditor::decrementAtCursor()
{
  ...
  case SequencerEditorMode::editingStep:
    ...
    std::vector<std::vector<double>> data = sequencer->getStepData(currentSequence, currentStep);
    if (data.size() > 1)
    data.pop_back(); writeStepData(data); // only shrink if small enough
    break;
  }
}
