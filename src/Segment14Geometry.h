...
public:
    ...
    explicit Segment14Geometry(Params p = {}) : params(p)
    {
        buildDefaultMap();
        buildSegmentRects();
    }
...
private:
    ...
    void buildDefaultMap()
    {
        ...
        charMap['B'] = bits({C,D,E,F,G1,G2,L,M});                 // “B” approximation
        charMap['C'] = bits({A,D,E,F});
        charMap['D'] = bits({B,C,D,E,G1,G2,L,M});                 // “D” approximation
        ...
        charMap['K'] = bits({E,F,H,J,L,M});                       // diagonals + center stem
        ...
        charMap['V'] = bits({E,F,J,K});                           // lower diagonals
    ...
    {
        const float W = params.cellW;
        const float H = params.cellH;
        ...
        const float left   = in;
        const float right  = W - in;
        const float top    = H - in;
        ...
        const float diagInsetX = in + t*0.6f;
        const float diagInsetY = in + t*0.6f;
